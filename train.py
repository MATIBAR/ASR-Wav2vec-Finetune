...
from utils.utils import set_seed  # Ensure utils module is correctly imported or adjusted
from utils.metric import Metric  # Adjust metric import based on your project structure
from dataloader.dataset import DefaultCollate  # Adjust dataset import based on your project
...
def main(rank, world_size, config, resume, preload):
    ...
    tokenizer = Wav2Vec2CTCTokenizer("vocab.json", **config["special_tokens"], word_delimiter_token="|")
    feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained("your_pretrained_model_path_here")
    processor = Wav2Vec2Processor(feature_extractor=feature_extractor, tokenizer=tokenizer)
    default_collate = DefaultCollate(processor, config['meta']['sr'])

    train_ds = train_base_ds.get_data()
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_ds, num_replicas=world_size, rank=rank, **config["train_dataset"]["sampler"])
    train_dl = DataLoader(dataset=train_ds, **config["train_dataset"]["dataloader"], sampler=train_sampler, collate_fn=default_collate)

    val_base_ds = initialize_module(config["val_dataset"]["path"], args=config["val_dataset"]["args"])
    val_ds = val_base_ds.get_data()
    val_sampler = torch.utils.data.distributed.DistributedSampler(val_ds, num_replicas=world_size, rank=rank, **config["val_dataset"]["sampler"])
    val_dl = DataLoader(dataset=val_ds, **config["val_dataset"]["dataloader"], sampler=val_sampler, collate_fn=default_collate)

    model = Wav2Vec2ForCTC.from_pretrained(
        "your_pretrained_model_path_here",
    ...
    if rank == 0:
        print(f"Number of training utterances: {len(train_ds)}")
        print(f"Number of validation utterances: {len(val_ds)}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--resume', action="store_true", help='path to latest checkpoint')
    parser.add_argument('-p', '--preload', default=None, type=str, help='Path to pretrained model')
